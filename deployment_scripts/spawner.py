...
class Vm(Vcenter_base, Vcenter_obj_tpl):
    ...
    def enable_startup(self, name):
        ...
        if not vm_obj:
            ...
            return
        cluster_obj = vm_obj.summary.runtime.host
        if not cluster_obj:
            self.logger.warning('Cluster({}) does not exist. Skip adding to startup VM: {}.'.format(cluster_obj.name, name))
            return
        spec = self.startup_info(vm_obj, cluster_obj)
        task = cluster_obj.configManager.autoStartManager.ReconfigureAutostart(spec)
        self.wait_for_tasks(self.service_instance, [task])

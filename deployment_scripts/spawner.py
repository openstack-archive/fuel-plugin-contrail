...
class Vcenter_base(object):
    def __init__(self, user_data=None, si=None):
        self.env_id, self.spawn, self.map_ips = self.get_args()
        if user_data:
        ...
        if self.exists():
            with open(self.file, 'r+') as f:
                ...
                for i, v in enumerate(content[self.data_key]):
                    ...
                    if not node_obj:
                        print 'Node with mac: {} not found.'.format(mac)
                        continue
...
if __name__ == '__main__':
    ...
    host_list = [{'host': host, 'uplink': uplink_name} for host in hosts_name]
    vmware_data = vcenter_connect.fetch_hosts_data()

    # Specify where save vmware_data
    vmware_datastore_file = '/var/www/nailgun/plugins/contrail-5.0/deployment_scripts/puppet/modules/contrail/files/vmware_data_{}.yaml'.format(
        str(vcenter_connect.env_id))
    vmware_datastore = Vcenterdata(vmware_datastore_file)
    # Save vmware data
    vmware_datastore.put(vmware_data)

    if vcenter_connect.spawn:
        dvs = Dvs(si=si)
        dvpg = Dvpg(si=si)

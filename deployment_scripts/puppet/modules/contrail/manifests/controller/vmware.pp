...
class contrail::controller::vmware {
  ...
  if $contrail::use_vcenter {
    ...
    exec {'retrive latest install packages':
      command => "/usr/bin/curl -fLO http://${::contrail::master_ip}:8080/plugins/contrail-4.0/latest-contrail-install-packages.deb",
    ...
    exec {'retrive vmdk':
      command => "/usr/bin/curl -fLO http://${::contrail::master_ip}:8080/plugins/contrail-4.0/ContrailVM-disk1.vmdk",

...
class contrail::vmware_integration {
  ...
  if $::use_vcenter == true {
    ...
    exec {'retrive install packages':
      command => "/usr/bin/wget -q http://${::master_ip}/plugins/contrail-3.0/latest-contrail-install-packages.deb -O /opt/contrail/contrail-install-packages.deb",
      creates => '/opt/contrail/contrail-install-packages.deb'
    } ->

    exec {'retrive vmware plugin packages':
      command => "/usr/bin/wget -q http://${::master_ip}/plugins/contrail-3.0/latest-contrail-install-vcenter-plugin.deb -O /opt/contrail/contrail-install-vcenter-plugin.deb",

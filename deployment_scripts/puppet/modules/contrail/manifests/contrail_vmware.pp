...
if $contrail::use_vcenter and $contrail::provision_vmware_type == 'fuel'{
    ...
    $install_packages = ['contrail-install-packages', 'contrail-fabric-utils', 'contrail-setup', 'contrail-vrouter-dkms', 'contrail-vrouter-common', 'contrail-nova-vif', 'open-vm-tools', 'iproute2']

    l23network::l3::ifconfig { $vmware_iface_name: ipaddr => 'none' }

    sysctl::value { 'net.nf_conntrack_max': value => '1048576'} ->
    ...
    sysctl::value { 'net.ipv4.tcp_keepalive_intvl': value => '3'} ->
    sysctl::value { 'net.ipv4.tcp_keepalive_time': value => '30'} ->
    sysctl::value { 'net.ipv4.tcp_keepalive_probes': value => '8'} ->
    sysctl::value { 'net.ipv4.conf.all.arp_accept': value => '1'} ->
    sysctl::value { 'net.ipv4.tcp_retries2': value => '5'} ->
    sysctl::value { 'net.ipv4.conf.all.rp_filter': value => '2'} ->
    sysctl::value { 'net.ipv4.conf.default.rp_filter': value => '2'} ->
    sysctl::value { 'kernel.core_pattern': value => '/var/crashes/core.%e.%p.%h.%t'} ->
    ...
    file {'/etc/contrail/vrouter_nodemgr_param':
      ...
      content => template('contrail/vrouter_nodemgr_param.erb'),
    } ->
    ...
    exec { "register_contrailvm_vrouter":
      ...
      command => "python provision_vrouter.py --host_name ${::fqdn} --host_ip $self_ip --api_server_ip ${contrail_internal_vip} \
--oper add --admin_user ${keystone_admin_user} --admin_password ${keystone_admin_password} --admin_tenant_name ${service_tenant_name} \
--openstack_ip ${internal_vip}  && touch /opt/contrail/register_contrailvm_vrouter-DONE",

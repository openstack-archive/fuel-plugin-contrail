...
class contrail::compute::network {
  ...
  if $contrail::compute_dpdk_on_vf {
    ...
    if count(keys($sriov_hash)) > 1 {
      ...
      $bond_vfs = keys(prefix($sriov_hash, 'dpdk_vf_'))
      $slaves   = join($bond_vfs, ' ')

      file_line {"set_bond_slaves_${raw_phys_dev}":
      ...
      #   interfaces      => ['dpdk-vf-eno1', 'dpdk-vf-ens4f1'],
      #   mtu             => 1500,
      #   onboot          => true,
    ...
    if ($vlan_tag =~ /^\d*$/ and count(keys($sriov_hash)) == 1 ) {
      ...
      $dpdk_dev_name = "dpdk_vf_${pf_dev_name}"

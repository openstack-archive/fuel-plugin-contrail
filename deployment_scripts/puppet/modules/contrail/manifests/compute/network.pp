  ...
  if $contrail::compute_dpdk_on_vf {
    ...
    if count(keys($sriov_hash)) > 1 {
      ...
      exec {"destroy_old_${raw_phys_dev}":
        ...
        unless  => "grep dpdk-vf /etc/network/interfaces.d/ifcfg-${raw_phys_dev}",
      }
      ...
      Exec["destroy_old_${raw_phys_dev}"] -> Configure_vfs <||> -> Exec["create_${raw_phys_dev}"]

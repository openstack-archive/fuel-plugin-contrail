...
class contrail::compute::vmware_pkg {
  ...
  package { ['nova-compute', 'nova-compute-kvm', 'nova-common']: } ->
  ...
  exec { 'patch-python-nova-vmware':
    command => 'patch -p1 < nova-vmware.patch && touch python-nova-vmware-patch-DONE',
    cwd     => $patch_path,
    creates => "${patch_path}/python-nova-vmware-patch-DONE",
  }

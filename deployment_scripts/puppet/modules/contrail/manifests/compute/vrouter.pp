...
class contrail::compute::vrouter {
  ...
  if ($contrail::compute_dpdk_enabled and $contrail::compute_dpdk_on_vf) {
  ...
  if $contrail::compute_dpdk_on_vf {
    ...
    fail {'DPDKoVF error: SR-IOV not enabled in kernel. Vrouter manifest aborted':}  
  }
  ...
  if $contrail::compute_dpdk_enabled == true {
    contrail_vrouter_agent_config {
      ...
      'DEFAULT/physical_interface_address' : value => $phys_dev_pci;

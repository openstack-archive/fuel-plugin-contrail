...
class contrail::compute::nova {
  ...
  $cgroup_acl_string='["/dev/null", "/dev/full", "/dev/zero", "/dev/random", "/dev/urandom", \
"/dev/ptmx", "/dev/kvm", "/dev/kqemu", "/dev/rtc", "/dev/hpet", "/dev/vfio/vfio", "/dev/net/tun"]'
  ...
  if !defined(Class['nova::network::neutron']) {
    nova_config {

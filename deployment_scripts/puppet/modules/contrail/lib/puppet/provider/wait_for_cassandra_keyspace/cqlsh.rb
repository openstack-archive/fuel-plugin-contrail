...
Puppet::Type.type(:wait_for_cassandra_keyspace).provide(:cqlsh) do
  ...
  def ensure
    ...
    if not @state[:keyspaces].include?(resource[:name])
      ...
      if @state[:keyspaces].include?(resource[:name]) and @resource[:tables] and is_subset_of(@resource[:tables], @state[:tables])
        debug "State: ready"
        :ready
      else
        debug "State: present"
  ...
  def resource_ready(value)
    ...
    when :ready
      @state[:keyspaces].include?(resource[:name]) and is_subset_of(@resource[:tables], @state[:tables])
    end
  end
  ...
  def is_subset_of(large_list, list)
    (large_list & list).count == list.count
  end
end

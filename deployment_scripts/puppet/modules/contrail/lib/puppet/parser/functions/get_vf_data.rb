...
module Puppet::Parser::Functions
  newfunction(:get_vf_data, :type => :rvalue, :doc => <<-EOS
    ...
    if (File.exists?("/sys/class/net/#{pf_dev_name}/device/virtfn#{vf_number}/net"))
        ...
        function_add_data_to_yaml(["/etc/hiera/plugins/contrail.yaml", 'dpdk_vf_' + pf_dev_name, vf_data])
    elsif not function_hiera_hash(['dpdk_vf_' + pf_dev_name, {}]).empty?
      vf_data = function_hiera_hash(['dpdk_vf_' + pf_dev_name, {}])
    end

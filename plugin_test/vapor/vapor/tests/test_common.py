...
def test_metadata_service(security_group, port_steps,
    ...
    """
        ...
        Pass criteria: The output of the metadata script
        should be seen in the VM.
    """
    userdata = (
        u'#!/bin/sh\n'
        u'echo "TestMetadataService.'
        u'The time is now $(date -R)!" | tee /tmp/output.txt\n'
    )
    ...
    with server_steps.get_server_ssh(server,
        ...
        assert_that(res.stdout, contains_string('output.txt'))

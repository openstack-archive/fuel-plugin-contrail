...
def net_group_preparation(obj):
    ...
    logger.info('{}'.format(gateway))
    # preparing commands for gateway setting and  private interface updating
    commands = [
        'fuel network-group --set --network {0} --gateway {1}'.format(
            ids, gateway),
        'fuel network-group --set --network {0} --meta "'
        '{"name": {1}, "notation": "cidr", '
        '"render_type": null, "map_priority": 2, "configurable": true, '
        '"render_addr_mask": "internal", "vlan_start": null, '
        '"use_gateway": true, "cidr": {2}'.format(ids, net_name, cidr)]

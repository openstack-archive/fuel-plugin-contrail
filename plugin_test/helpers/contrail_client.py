...
class ContrailClient(object):
    ...
    def _get(self, data_path):
        logger.info('{0}'.format(self.url + data_path))
        logger.info('{0}'.format(data_path))
        return self.client.get(url=self.url + data_path).json()

    def _delete(self, data_path):
        return self.client.delete(url=self.url.join(data_path)).json()

    def _post(self, data_path, **kwargs):
        return self.client.post(
            url=self.url.join(data_path), connect_retries=1, **kwargs).json()

...
class BMDriver(object):
    ...
    def setup_fuel_node(self, obj, cluster_id, roles,
        ...
        node = self.get_bm_node(obj, self.conf['target_macs'])
    ...
    def update_node_interfaces(self, obj, node):
        ...
        target_macs = self.conf['target_macs']

        def get_pxe_interface(hw_interfaces):
            pxe = None
            for net in node['network_data']:
                if net['name'] == 'fuelweb_admin':
                    logger.debug("Found PXE interface: {0}".format(pxe))
                    pxe = net['dev']
            for interface in node['meta']['interfaces']:
                if (interface['mac'] in hw_interfaces and
                            interface['name'] != pxe):
                    logger.debug(
                        'pxe nets: {0} and {1}'.format(pxe, interface['name']))
                    return pxe, interface['name']
            return None, None

        pxe, hw = get_pxe_interface(target_macs)
        networks = {
            'eno1': ['public'],
            'eno2': [],
            pxe: ['fuelweb_admin', 'storage', 'management'],
            hw: ['private']
        }

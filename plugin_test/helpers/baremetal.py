...
class BridgeTool(object):
    ...
    def del_interface(bridge, interface):
        ...
        cmd = "brctl delif '{0}' '{1}'".format(bridge, interface)
        res = subprocess.check_output(cmd, shell=True).replace('\n', '')
        return res
    ...
    def add_interface(bridge, interface):
        ...
        cmd = "brctl addif '{0}' '{1}'".format(bridge, interface)
        res = subprocess.check_output(cmd, shell=True).replace('\n', '')
        return res
...
class BMDriver(object):
    ...
    def update_vm_node_interfaces(self, obj, cluster_id):
        ...
        nailgun_nodes = obj.client.list_cluster_nodes(cluster_id)
        baremetal_macs = self.conf['target_macs']
        for node in nailgun_nodes:
            if node['mac'] in baremetal_macs:
                # Skip Barematal node
                continue
            obj.client.update_node_networks(node['id'], assigned_networks)

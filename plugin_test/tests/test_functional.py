...
class IntegrationTests(TestBasic):
    ...
    def contrail_no_default(self):
        ...
        assert_true(
            int(contrail_external.split('/')[1]) == external_ip['ip_prefix'],
            message.format(
        ...
        bgp_id = contrail_client.get_bgp_routers()['bgp-routers'].pop()['uuid']

        bgp_ip = contrail_client.get_bgp_by_id(bgp_id)[
            'bgp-router']['bgp_router_parameters']['address']

        assert_true(
            contrail_gateways == bgp_ip,
            message.format(contrail_gateways, bgp_ip))

...
class SystemTests(TestBasic):
    ...
    def contrail_ceilometer_metrics(self):
        ...
        srv_2 = os_conn.create_server_for_migration(
            neutron=True, label='admin_internal_net')
        fip_2 = os_conn.assign_floating_ip(srv_2).ip

        for fip in [fip_1, fip_2]:
            wait(
        ...
        self.ping_instance_from_instance(
            os_conn, controller.name, {fip_1: [fip_2]})

        with self.fuel_web.get_ssh_for_node(controller.name) as ssh:
            for metric in ceilometer_metrics:
                m = ssh.execute(
                    'ceilometer sample-list -m {0}'.format(metric))['stdout']
                logger.info('{}'.format(m))
                assert_true(m > 0, '{0} is {1}'.format(metric, m))

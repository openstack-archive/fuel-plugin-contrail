...
class DPDKTests(TestBasic):
    ...
    def contrail_dpdk_boot_snapshot_vm(self):
        ...
        wait(lambda: os_conn.nova.images.get(image).status == 'ACTIVE',
             timeout=self.BOOT_TIMEOUT, timeout_msg='Image is not active.')
    ...
    @test(depends_on=[contrail_dpdk_setup],
          groups=[
              "contrail_dpdk_public_connectivity_from_instance_without_fip"])
    @log_snapshot_after_test
    def contrail_dpdk_public_connectivity_from_instance_without_fip(self):
        """Check network connectivity from instance via floating IP.

        Scenario:
            1. Setup contrail_dpdk_setup.
            2. Get existing flavor with hpgs.
            3. Create a new security group (if it doesn`t exist yet).
            4. Launch an instance using the default image and flavor with hpgs
               in the hpgs availability zone.
        ...
        access_point = os_conn.create_server_for_migration(
            neutron=True, availability_zone=az_name, flavor=flavor)
        access_point_fip = os_conn.assign_floating_ip(access_point).ip
        wait(
            lambda: tcp_ping(access_point_fip, 22), timeout=60, interval=5,

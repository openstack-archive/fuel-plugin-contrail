...
class TestContrailCheck(object):
    ...
    def __init__(self, obj):
        ...
        self.node_roles = [
            role for node in obj.fuel_web.client.list_nodes()
            for role in node['roles']]
        cluster_id = obj.fuel_web.get_last_created_cluster()
        ip = obj.fuel_web.get_public_vip(cluster_id)
    ...
    def _clear_openstack(self):
        ...
        router_id = [
            ...
            if router['name'] == self.router_name].pop()
        if instances:
        ...
        if router_id:
            try:
                ...
                self.os_conn.neutron_client.delete_router(router_id)
            except Exception as exc:
                logger.info('Routerr was not deleted. {0}'.format(exc))
        if subnet_id:
            try:
    ...
    def test_contrail_dpdk_volume(self):
        ...
        """
        if not 'cinder' in self.node_roles:
            raise SkipTest()
        az_name = 'hpgs'
    ...
    def test_contrail_dpdk_check_public_connectivity_from_instance(self):
        ...
        router = self.os_conn.neutron.create_router(body=router_param)['router']
    ...
    def _verify_instance_state(self, instances=None,
        ...
        for instance in instances:
            try:
                wait(
                    lambda:
                   self.os_conn.get_instance_detail(

...
class TestContrailCheck(object):
    ...
    def _run_tests(self, set_tests):
        ...
        for test_name, test_method in tests:
            try:
                ...
                if exc == 'SkipTest':
                    logger.info('{0} skipped with {1}'.format(test_name, exc))
    ...
    def _remove_security_groups(self):
        ...
        for group in security_groups:
            try:
                ...
                logger.info('Security group {0} was not deleted. {1}'.format(
                    group['name'], exc))
    ...
    def test_contrail_dpdk_check_public_connectivity_from_instance(self):
        ...
        logger.info("""Launch an instance using the default image and flavor
            with hpg in the hpgs availability zone.""")

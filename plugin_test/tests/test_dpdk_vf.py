...
class DPDKVFTests(TestBasic):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["contrail_dpdk_sriov_setup"])
    @log_snapshot_after_test
    def contrail_dpdk_vf_setup(self):
        """Contrail DPDK setup.
    ...
    @test(depends_on=[contrail_dpdk_vf_setup],
          groups=["contrail_dpdk_vf_boot_snapshot_vm"])
    @log_snapshot_after_test
    def contrail_dpdk_vf_boot_snapshot_vm(self):
    ...
    @test(depends_on=[contrail_dpdk_vf_setup],
          groups=["contrail_dpdk_vf_volume"])
    @log_snapshot_after_test
    def contrail_dpdk_vf_volume(self):

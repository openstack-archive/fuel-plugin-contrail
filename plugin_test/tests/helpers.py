...
def node_rename(fuel_web, node_name_regexp, new_name, update_all=False):
    ...
    for node in configured_nodes:
        if 'contrail' in node['name']:
            nodes4rename.append(node)
    ...
    for node in configured_nodes:
        ...
        # if node_name_ptrn in node['name']:
        if re_obj.match(node['name']):
            logger.info("RENAME NODE :%s -> %s" % (node['name'], new_name))
    ...
    logger.info('#'*80)
    logger.info('Nodes4rename:')
    from pprint import pformat
    logger.info(pformat(nodes4update))
    logger.info('#'*80)

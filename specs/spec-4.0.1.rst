...
provides are not available with DPDK. Also because of binding interface not be
seen by the kernel, user can't reuse it. On environment with difficult network schema or on servers with low amount of network interfaces it can be significant disadvantage.

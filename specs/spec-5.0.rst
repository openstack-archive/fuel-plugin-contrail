...
Contrail plugin 3.0 implements deployment of Contrail analytics services to dedicated nodes with
...
the same database. The recommended best practice from Juniper for production deployment of Contrail
is to separate database for Contrail Config and Contrail Analytics. The Contrail Analytics along
with Analytics DB would be installed on a separate node (physical host or VM) while Contrail Config
along with Config DB would be a separate node [0].
...
their manual decomission is not supported.
...
Experimental scripts for contrail packages upgrade should be updated with upgrade tasks for
